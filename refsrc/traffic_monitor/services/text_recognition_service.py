...
# Support both new and old FastPlateOCR APIs
try:
    from fast_plate_ocr import LicensePlateRecognizer as _PlateRecognizer  # noqa: F401
except ImportError:  # pragma: no cover
    from fast_plate_ocr import ONNXPlateRecognizer as _PlateRecognizer  # type: ignore
...
class TextRecognitionService:
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.backend == "fast_plate_ocr":
            ...
            try:
                self.reader = _PlateRecognizer(hub_ocr_model=hub_model_name, device=device)
