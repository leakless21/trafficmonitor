...
from ..utils.custom_types import TrackedObject
...
class VisualizationService:
    ...
    def _parse_colors(self, colors_config):
        ...
        return {k: _fast_parse_color(v) for k, v in colors_config.items()}
...
def _fast_parse_color(color):
    """Optimized color parsing helper (list, tuple, or str '(r,g,b)')."""
    if isinstance(color, tuple):
        return color
    elif isinstance(color, list):
        # Array from config, common case
        try:
            return tuple(color)
        except Exception:
            return (255, 255, 255)
    elif isinstance(color, str):
        # Fastest route on '(x,y,z)'
        if color and color[0] == '(' and color[-1] == ')':
            try:
                parts = color[1:-1].split(',')
                if len(parts) == 3:
                    return (int(parts[0]), int(parts[1]), int(parts[2]))
            except Exception:
                pass
        # fallback on any other string
        return (255, 255, 255)
    # fallback
    return (255, 255, 255)
